...
const TagsPage = () => {
  ...
  const fetchTags = async () => {
    try {
      ...
      setSnackbar({ 
        open: true, 
        message: `Error fetching tags: ${error instanceof Error ? error.message : 'Unknown error'}`, 
        severity: 'error' 
      })
      setTags([])
    } finally {
  ...
  const handleDeleteTag = async () => {
    ...
    try {
      ...
      setSnackbar({ open: true, message: 'Error deleting tag', severity: 'error' })
    }
  }
  ...
  const toggleTagStatus = async (tag: Tag) => {
    try {
      ...
      setSnackbar({ 
        open: true, 
        message: `Tag ${!tag.is_active ? 'activated' : 'deactivated'} successfully`, 
        severity: 'success' 
      })
      fetchTags()
    } catch (error) {
      console.error('Error updating tag status:', error)
      setSnackbar({ open: true, message: 'Error updating tag status', severity: 'error' })
    }
  }
  ...
  const handleSaveTag = async (tagData: any) => {
    try {
      ...
      setSnackbar({ open: true, message: 'Error saving tag', severity: 'error' })
    }
  }
  ...
  const copyTagSlug = (slug: string) => {
    ...
    setSnackbar({ open: true, message: 'Tag slug copied to clipboard', severity: 'success' })
  }
        ...
        <Box sx={{ p: 3 }}>
          ...
          <Card sx={{ mb: 3, p: 3, borderRadius: 2, boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
              ...
              >
                <FilterIcon sx={{ color: THEME_COLORS.rentPrimary, fontSize: 20 }} />
              </Box>
              <Box>
          ...
          <Card sx={{ borderRadius: 2, boxShadow: '0 2px 8px rgba(0,0,0,0.1)', overflow: 'hidden' }}>
            <Box sx={{ p: 3, borderBottom: '1px solid #e0e0e0', backgroundColor: THEME_COLORS.tint }}>
              ...
              <Table>
                <TableHead>
                  <TableRow sx={{ backgroundColor: '#f8f9fa' }}>
                    <TableCell sx={{ fontWeight: 'bold', color: THEME_COLORS.rentPrimary }}>Tag</TableCell>
                    <TableCell sx={{ fontWeight: 'bold', color: THEME_COLORS.rentPrimary }}>Description</TableCell>
                    <TableCell sx={{ fontWeight: 'bold', color: THEME_COLORS.rentPrimary }}>Usage</TableCell>
                    <TableCell sx={{ fontWeight: 'bold', color: THEME_COLORS.rentPrimary }}>Sort Order</TableCell>
                    <TableCell sx={{ fontWeight: 'bold', color: THEME_COLORS.rentPrimary }}>Status</TableCell>
                    <TableCell sx={{ fontWeight: 'bold', color: THEME_COLORS.rentPrimary }}>Actions</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  {loading ? (
                      ...
                      <TableRow key={tag.tag_id} hover>
                        <TableCell>
                          <Box sx={{ display: 'flex', alignItems: 'center' }}>
                            ...
                            <Box sx={{ flex: 1 }}>
                              <Typography variant="subtitle2" sx={{ fontWeight: 'bold', mb: 0.5, color: THEME_COLORS.rentPrimary }}>
                                {tag.name}
                              </Typography>
                          ...
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            <Box
                              sx={{
                                ...
                                height: 32,
                                borderRadius: '50%',
                                backgroundColor: tag.usage_count > 0 ? THEME_COLORS.primary : '#e0e0e0',
                                display: 'flex',
                                alignItems: 'center',
                                justifyContent: 'center',
                                color: 'white',
                            ...
                            <Typography variant="body2" color="text.secondary">
                              {tag.usage_count === 0 ? 'Not used' : 
                               tag.usage_count === 1 ? '1 product' : 
          ...
          <Typography>
            ...
            {tagToDelete?.usage_count > 0 && (
              <Alert severity="warning" sx={{ mt: 2 }}>
                This tag is currently used by {tagToDelete.usage_count} products. Deleting it will remove the tag from all products.
              </Alert>
            )}
          </Typography>
        </DialogContent>
