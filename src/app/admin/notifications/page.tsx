...
const NotificationsPage = () => {
  ...
  const [notificationToDelete, setNotificationToDelete] = useState<NotificationHistory | null>(null)
  ...
  const handleDeleteNotification = async () => {
    ...
    try {
      ...
      setNotificationHistory(prev => prev.filter(n => n.id !== notificationToDelete.id))
  ...
  const handleSendNotification = async () => {
    try {
      ...
      if (!formData.message.trim()) {
        setSnackbar({ open: true, message: 'General message is required', severity: 'error' })
        return
      }
      if (formData.types.length === 0) {
        setSnackbar({ open: true, message: 'Please select at least one notification type', severity: 'error' })
        return
      }
      ...
      if (formData.types.includes('Email')) {
        if (!formData.email_subject.trim()) {
          setSnackbar({ open: true, message: 'Email subject is required', severity: 'error' })
          return
        }
        if (!formData.email_content.trim()) {
          setSnackbar({ open: true, message: 'Email content is required', severity: 'error' })
          return
        }
      }
      ...
      if (formData.types.includes('SMS')) {
        if (!formData.sms_content.trim()) {
          setSnackbar({ open: true, message: 'SMS content is required', severity: 'error' })
          return
        }
      }
      ...
      if (formData.types.includes('Mobile notification')) {
        if (!formData.push_title.trim()) {
          setSnackbar({ open: true, message: 'Push notification title is required', severity: 'error' })
          return
        }
        if (!formData.push_content.trim()) {
          setSnackbar({ open: true, message: 'Push notification content is required', severity: 'error' })
          return
        }
      }
      
      if (formData.schedule_type === 'scheduled' && (!formData.scheduled_date || !formData.scheduled_time)) {
        setSnackbar({ open: true, message: 'Please set scheduled date and time', severity: 'error' })
        return
      }
      ...
      setSnackbar({ 
        open: true, 
        message: formData.schedule_type === 'now' ? 'Notification sent successfully!' : 'Notification scheduled successfully!', 
        severity: 'success' 
      })
  ...
  const getEstimatedRecipientCount = () => {
    switch (formData.target_audience) {
      case 'all': return appUsers.length
      case 'specific': return formData.specific_users.length
      default: return 0
    }
  }
  ...
  const getStatusChip = (status: string) => {
    switch (status) {
      ...
      case 'sending':
        return <Chip icon={<PendingIcon />} label="Sending" color="warning" size="small" />
      case 'scheduled':
        return <Chip icon={<ScheduleIcon />} label="Scheduled" color="info" size="small" />
      case 'failed':
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'SMS': return <SmsIcon sx={{ fontSize: 16 }} />
      case 'Mobile notification': return <MobileIcon sx={{ fontSize: 16 }} />
  ...
  const getSuccessRate = (notification: NotificationHistory) => {
    ...
    return Math.round((notification.sent_count / notification.recipient_count) * 100)
  }
  ...
  return (
    <Box sx={{ display: 'flex', minHeight: '100vh' }}>
        ...
        <Box sx={{ p: 3 }}>
          ...
          <Grid container spacing={3} sx={{ mb: 3 }}>
            ...
            <Grid item xs={12} sm={6} md={3}>
              <Card elevation={3}>
                <Box sx={{ p: 2, display: 'flex', alignItems: 'center' }}>
                  <CheckCircleIcon sx={{ color: 'success.main', mr: 2, fontSize: 32 }} />
                  <Box>
                ...
                <Box sx={{ p: 2, display: 'flex', alignItems: 'center' }}>
                  ...
                  <Box>
                    <Typography variant="h4" sx={{ color: 'warning.main', fontWeight: 'bold' }}>
                      {Math.round(notificationHistory.reduce((acc, n) => acc + getSuccessRate(n), 0) / Math.max(notificationHistory.length, 1))}%
                    </Typography>
                    <Typography variant="body2" color="text.secondary">
                      Success Rate
                    </Typography>
                  ...
                  {loading ? (
                      ...
                      <TableCell colSpan={8} sx={{ textAlign: 'center', py: 8 }}>
                        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                          ...
                          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                            Start by creating and sending your first notification to users
                          </Typography>
                          <Button
                            variant="contained"
                            startIcon={<SendIcon />}
                    ...
                    paginatedNotifications.map((notification) => (
                      <TableRow key={notification.id} hover>
                        ...
                        <TableCell>
                          <Box sx={{ display: 'flex', alignItems: 'center' }}>
                            {notification.target_audience === 'all' && <GroupIcon sx={{ fontSize: 16, mr: 1, color: 'text.secondary' }} />}
                            {notification.target_audience === 'specific' && <PersonIcon sx={{ fontSize: 16, mr: 1, color: 'text.secondary' }} />}
                            ...
                            <LinearProgress
                              ...
                              sx={{
                                ...
                                '& .MuiLinearProgress-bar': {
                                  backgroundColor: getSuccessRate(notification) >= 90 ? '#4caf50' : 
                                                 getSuccessRate(notification) >= 70 ? '#ff9800' : '#f44336'
                                }
                              }}
                            />
      ...
      <Dialog open={deleteConfirmOpen} onClose={() => setDeleteConfirmOpen(false)}>
        ...
        <DialogContent>
          <Typography>
            Are you sure you want to delete notification "{notificationToDelete?.title}"? This action cannot be undone.
          </Typography>
        </DialogContent>
        <DialogActions>
      ...
      >
        ...
        <DialogContent sx={{ p: 0, maxHeight: '75vh', overflowY: 'auto' }}>
          ...
          <Box sx={{ p: 0 }}>
                  ...
                  <TextField
                    ...
                    sx={{
                      '& .MuiOutlinedInput-root': {
                        ...
                        '&.Mui-focused fieldset': { borderColor: THEME_COLORS.rentPrimary },
                      },
                    }}
                  />
                </Grid>
            ...
            <Box sx={{ p: 3, borderBottom: '1px solid #e0e0e0' }}>
              ...
              {/* Notification Types */}
              <Box sx={{ mb: 4 }}>
                <Typography variant="subtitle1" sx={{ fontWeight: 'bold', mb: 2, color: THEME_COLORS.rentPrimary }}>
                ...
                <Grid container spacing={2}>
                    ...
                    <FormControlLabel
                      control={
                        <Checkbox
                          ...
                          onChange={(e) => handleTypeChange('Email', e.target.checked)}
                    ...
                    <FormControlLabel
                      control={
                        <Checkbox
                          ...
                          onChange={(e) => handleTypeChange('SMS', e.target.checked)}
                          sx={{ color: THEME_COLORS.rentPrimary }}
                        />
                      }
                      label={
                        <Box sx={{ display: 'flex', alignItems: 'center' }}>
                          ...
                          <Box>
                            <Typography variant="body2" sx={{ fontWeight: 'bold' }}>SMS</Typography>
                            <Typography variant="caption" color="text.secondary">
                  ...
                  <Grid item xs={12} sm={4}>
                    <FormControlLabel
                      control={
                        <Checkbox
                          checked={formData.types.includes('Mobile notification')}
                          onChange={(e) => handleTypeChange('Mobile notification', e.target.checked)}
                          sx={{ color: THEME_COLORS.rentPrimary }}
                        />
                      }
                      label={
                        <Box sx={{ display: 'flex', alignItems: 'center' }}>
                          ...
                          <Box>
                            <Typography variant="body2" sx={{ fontWeight: 'bold' }}>Mobile Push</Typography>
                            <Typography variant="caption" color="text.secondary">
              ...
              <Box>
                <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                  <Typography variant="subtitle1" sx={{ fontWeight: 'bold', color: THEME_COLORS.rentPrimary }}>
                    Target Audience
                  </Typography>
                  <Badge 
                ...
                >
                  <FormControlLabel 
                    ...
                    label={
                      <Box>
                        <Typography variant="body2" sx={{ fontWeight: 'bold' }}>All App Users</Typography>
                        <Typography variant="caption" color="text.secondary">
                  ...
                  <FormControlLabel 
                    ...
                    label={
                      <Box>
                        <Typography variant="body2" sx={{ fontWeight: 'bold' }}>Specific Users</Typography>
                        <Typography variant="caption" color="text.secondary">
                  ...
                  <Box sx={{ mt: 2 }}>
                    <FormControl fullWidth>
                      ...
                      <Select
                        ...
                        onChange={(e) => setFormData({ ...formData, specific_users: e.target.value as string[] })}
                        label="Select Users"
                              ...
                              return (
                                <Chip
                                  ...
                                  onDelete={() => {
                                    setFormData({
                                      ...formData,
                                      specific_users: formData.specific_users.filter(id => id !== userId)
                                    })
                                  }}
                  ...
                  <Box sx={{ mb: 4 }}>
                    ...
                    <Box sx={{ mb: 3 }}>
                      ...
                      <TextField
                        ...
                        placeholder={formData.message || "Enter email content..."}
                        sx={{
                          '& .MuiOutlinedInput-root': {
                            '&:hover fieldset': { borderColor: THEME_COLORS.primary },
                            '&.Mui-focused fieldset': { borderColor: THEME_COLORS.rentPrimary },
                  ...
                  <Box sx={{ mb: 4 }}>
                    ...
                    <Box sx={{ mb: 3 }}>
                      ...
                      <TextField
                        ...
                        onChange={(e) => setFormData({ ...formData, push_content: e.target.value })}
                        required
                        multiline
                        rows={3}
                        helperText="Push notification body content"
                        placeholder={formData.message || "Enter push content..."}
                        sx={{
                          mb: 2,
                          '& .MuiOutlinedInput-root': {
                            '&:hover fieldset': { borderColor: THEME_COLORS.primary },
                      ...
                      <TextField
                        ...
                        onChange={(e) => setFormData({ ...formData, push_icon: e.target.value })}
                ...
                <Box sx={{ mt: 3, p: 2, backgroundColor: '#f8f9fa', borderRadius: 1 }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                    <Button
                      ...
                      onClick={() => {
                        const updates: any = {}
                        if (formData.types.includes('Email') && !formData.email_content) {
                          updates.email_content = formData.message
                        }
                        if (formData.types.includes('SMS') && !formData.sms_content) {
                          updates.sms_content = formData.message
                        }
                        if (formData.types.includes('Mobile notification') && !formData.push_content) {
                    ...
                    <Button
                      ...
                      onClick={() => {
                        const updates: any = {}
                        if (formData.types.includes('Email') && !formData.email_subject) {
                          updates.email_subject = formData.title
                        }
                        if (formData.types.includes('Mobile notification') && !formData.push_title) {
            ...
            <Box sx={{ p: 3, borderBottom: '1px solid #e0e0e0' }}>
              ...
              >
                <FormControlLabel 
                  ...
                  label={
                    <Box>
                      <Typography variant="body2" sx={{ fontWeight: 'bold' }}>Send Immediately</Typography>
                      <Typography variant="caption" color="text.secondary">
                ...
                <FormControlLabel 
                  ...
                  label={
                    <Box>
                      <Typography variant="body2" sx={{ fontWeight: 'bold' }}>Schedule for Later</Typography>
                      <Typography variant="caption" color="text.secondary">
              ...
              <FormControlLabel
                control={
                  <Switch
                    ...
                    onChange={(e) => setFormData({ ...formData, include_unsubscribed: e.target.checked })}
                    sx={{
                ...
                label={
                  <Box>
                    ...
                    <Typography variant="caption" color="text.secondary">
                      Only enable for important system notifications (account security, service updates, etc.)
                    </Typography>
                  </Box>
                }
              />
        ...
        <DialogActions sx={{ p: 3, backgroundColor: '#f8f9fa' }}>
          ...
          >
            {sending ? 'Sending...' : (formData.schedule_type === 'now' ? 'Send Now' : 'Schedule')}
          </Button>
        </DialogActions>
      </Dialog>
