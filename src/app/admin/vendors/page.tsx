    ...
    try {
      if (editingVendor) {
        ...
        try {
          ...
          if (!emailResponse.ok) {
            console.error('Failed to send welcome email, but vendor was created successfully')
        ...
        setSnackbar({ open: true, message: 'Vendor created successfully and welcome email sent!', severity: 'success' })
      }
      ...
      const errorMessage = error instanceof Error ? error.message : 'Error saving vendor'
    ...
    try {
      ...
      if (productIds.length > 0) {
        const { data: orderItems, error: orderItemsError } = await supabase
          .from('order_items')
          .select('order_id, orders(order_id, order_number, total_amount, created_at, status)')
      ...
      const { data: earnings, error: earningsError } = await supabase
        ...
        .select('earning_id, gross_amount, commission_amount, net_amount, status')
        .eq('vendor_id', vendorId)
      ...
      setSnackbar({ open: true, message: 'Error loading vendor data', severity: 'error' })
    } finally {
  ...
  const handleDeleteVendor = async () => {
    ...
    try {
      ...
      if (productIds.length > 0) {
        ...
        orderIds = [...new Set(orderItems?.map((oi: any) => oi.order_id) || [])]
      }
      ...
      setSnackbar({ open: true, message: 'Vendor and all related data deleted successfully', severity: 'success' })
      ...
      setSnackbar({ open: true, message: 'Error deleting vendor', severity: 'error' })
    } finally {
      setLoading(false)
    }
  }
                  ...
                  {loading ? (
                    ...
                    paginatedVendors.map((vendor) => (
                      <TableRow key={vendor.vendor_id} hover>
                        <TableCell>
                          <Box sx={{ display: 'flex', alignItems: 'center' }}>
                            <Avatar sx={{ mr: 2, bgcolor: THEME_COLORS.primary }}>
                        ...
                        <TableCell>
                          <Box>
                            ...
                            <Chip
                              icon={<VerifiedIcon />}
                              label={vendor.is_verified ? 'Verified' : 'Unverified'}
            ...
            <>
              <Alert severity="error" sx={{ mb: 3 }}>
                <Typography variant="body1" sx={{ fontWeight: 'bold', mb: 1 }}>
                  Warning: All data related to vendor "{vendorToDelete?.name}" will be permanently deleted!
                </Typography>
                <Typography variant="body2">
                  This includes all products, orders, earnings, coupons, and reviews associated with this vendor.
                </Typography>
              </Alert>
                ...
                <CardContent>
                  <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                    <Avatar sx={{ bgcolor: '#ff9800', mr: 2, width: 32, height: 32 }}>
                      <Typography sx={{ fontWeight: 'bold' }}>{vendorDeleteData.products.length}</Typography>
                    </Avatar>
                    <Typography variant="h6" sx={{ fontWeight: 'bold' }}>
                      ...
                      {vendorDeleteData.products.map((product) => (
                        ...
                        >
                          ...
                          <Typography variant="body2" color="text.secondary">
                            ₹{product.price_per_day}/day (Stock: {product.stock_quantity})
                          </Typography>
                        </Box>
                      ))}
                    </Box>
                ...
                <CardContent>
                  <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                    <Avatar sx={{ bgcolor: '#2196f3', mr: 2, width: 32, height: 32 }}>
                      <Typography sx={{ fontWeight: 'bold' }}>{vendorDeleteData.orders.length}</Typography>
                    </Avatar>
                    <Typography variant="h6" sx={{ fontWeight: 'bold' }}>
                ...
                <CardContent>
                  <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                    <Avatar sx={{ bgcolor: '#4caf50', mr: 2, width: 32, height: 32 }}>
                      <Typography sx={{ fontWeight: 'bold' }}>{vendorDeleteData.earnings.length}</Typography>
                    </Avatar>
                    <Typography variant="h6" sx={{ fontWeight: 'bold' }}>
                  ...
                  {vendorDeleteData.earnings.length > 0 ? (
                    <Box>
                      <Typography variant="body2" color="text.secondary">
                        Total earnings: ₹
                        {vendorDeleteData.earnings.reduce((sum, e) => sum + parseFloat(e.net_amount || 0), 0).toFixed(2)}
                      </Typography>
                    </Box>
                  ) : (
                    <Typography variant="body2" color="text.secondary">No earnings found</Typography>
                  )}
                </CardContent>
              </Card>
              ...
              <Alert severity="warning" sx={{ mt: 3 }}>
                <Typography variant="body2">
                  Additionally, all related reviews, product highlights, wishlist items, coupons, and coupon usage records will also be deleted.
                </Typography>
              </Alert>
            </>
          )}
