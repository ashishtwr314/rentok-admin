...
const ProductsPage = () => {
  ...
  const handleDeleteProduct = async () => {
    ...
    try {
      ...
      if (productToDelete.images && productToDelete.images.length > 0) {
        const imagesToDelete = productToDelete.images.filter(
          (imageUrl: string) =>
  ...
  const toggleDealOfTheDayStatus = async (product: Product) => {
    try {
      ...
      setSnackbar({
        ...
        message: `Product ${
          !product.deal_of_the_day
            ? "marked as deal of the day"
            : "removed from deal of the day"
        } successfully`,
        severity: "success",
      });
      fetchData();
      ...
      setSnackbar({
        open: true,
        message: "Error updating deal of the day status",
        severity: "error",
      });
    }
  };
  ...
  const handleSaveProduct = async (productData: any) => {
    try {
      ...
      const cleanedData = {
        ...productData,
        vendor_id:
          productData.vendor_id && productData.vendor_id.trim() !== ""
    ...
    <Box sx={{ display: "flex", minHeight: "100vh" }}>
                              ...
                              >
                                ...
                                {product.deal_of_the_day && (
                                  <Chip
                                    ...
                                    size="small"
                                    onClick={() =>
                                      toggleDealOfTheDayStatus(product)
                                    }
                                    sx={{ cursor: "pointer" }}
                                  />
                                )}
      ...
      >
                  ...
                  >
                    ...
                    <Box sx={{ display: "flex", gap: 4 }}>
                      <Box>
                        ...
                        <Typography variant="body1">
                          {selectedProduct.security_deposit
                            ? `₹${selectedProduct.security_deposit}`
                            : "Not required"}
                        </Typography>
                      </Box>
        ...
        <DialogActions sx={{ p: 3, backgroundColor: "#f8f9fa" }}>
          <Button onClick={() => setViewDetailsOpen(false)} variant="outlined">
            Close
          </Button>
          <Button
            variant="contained"
